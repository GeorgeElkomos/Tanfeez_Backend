...
def submit_automatic_posting(ledger_id: str = "300000288873799") -> str:
    """
    Submit Automatic Posting job for General Ledger
    
    Args:
        ledger_id: The ledger ID to post to (default: 300000288873799)
   ...
   <soapenv:Body>
    ...
    if r.status_code != 200:
        raise SystemExit(f"submitESSJobRequest (Automatic Posting) HTTP {r.status_code}")
...
if __name__ == "__main__":
    ...
    job_id = submit_automatic_posting("300000288873799")
    
    print(f"\n🎉 Automatic Posting job submitted with ID: {job_id}")
    print("💡 You can check the job status in Oracle Fusion or use the get_ess_status function.")